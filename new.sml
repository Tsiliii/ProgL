...
fun savethecat file =
	let
		...
		fun update_c(arr,time_array,cats,water,i:int,j:int,c:char,t,n,m,cats_num,water_num,water_prev) =
			if j<=n andalso j>=0 andalso i<=m andalso i>=0 then
				let
					val	x = Cmap.find(arr,(i,j))
					val	y = Cmap.find(time_array, (i,j))
				in
					case c of #"R" =>
						if valOf(x) = #"." then update_c(Cmap.insert(arr,(i,j),c),Cmap.insert(time_array, (i,j),t),pf(cats,i,j,t+1),water,i,j-2,#"L",t,n,m,cats_num+1,water_num,water_prev)
						else if  is_cat(valOf(x)) andalso valOf(y) = t then update_c(Cmap.insert(arr,(i,j),conflict(c,valOf(x))),Cmap.insert(time_array, (i,j),t),cats,water,i,j-2,#"L",t,n,m,cats_num,water_num,water_prev)
					else update_c(arr,time_array,cats,water,i,j-2,#"L",t,n,m,cats_num,water_num,water_prev)
					| #"L" =>
						if  valOf(x) = #"." then update_c(Cmap.insert(arr,(i,j),c),Cmap.insert(time_array, (i,j),t),pf(cats,i,j,t+1),water,i+1,j+1,#"D",t,n,m,cats_num+1,water_num,water_prev)
						else if is_cat(valOf(x)) andalso valOf(y) = t then if t = 2 then print("5") else update_c(Cmap.insert(arr,(i,j),conflict(c,valOf(x))),Cmap.insert(time_array, (i,j), t+1),cats,water,i+1,j+1,#"U",t,n,m,cats_num,water_num,water_prev)
						else update_c(arr,time_array,cats,water,i+1,j+1,#"U",t,n,m,cats_num,water_num,water_prev)
					| #"D" =>
						if  valOf(x) = #"." then update_c(Cmap.insert(arr,(i,j),c),Cmap.insert(time_array, (i,j),t),pf(cats,i,j,t+1),water,i-2,j,#"U",t,n,m,cats_num+1,water_num,water_prev)
						else if  is_cat(valOf(x)) andalso valOf(y) = t then update_c(Cmap.insert(arr,(i,j),conflict(c,valOf(x))),Cmap.insert(time_array, (i,j),t),cats,water,i-2,j,#"D",t,n,m,cats_num,water_num,water_prev)
						else update_c(arr,time_array,cats,water,i-2,j,#"D",t,n,m,cats_num,water_num,water_prev)
					| #"U" =>
						if  valOf(x) = #"." then update_cat(Cmap.insert(arr,(i,j),c),Cmap.insert(time_array, (i,j),t),pf(cats,i,j,t+1),water,t,n,m,cats_num+1,water_num,water_prev)
						else if  is_cat(valOf(x)) andalso valOf(y) = t then update_cat(Cmap.insert(arr,(i,j),conflict(c,valOf(x))),Cmap.insert(time_array, (i,j),t),cats,water,t,n,m,cats_num,water_num,water_prev)
			...
			| #"L" =>
				update_c(arr,time_array,cats,water,i+1,j+1,#"D",t,n,m,cats_num,water_num,water_prev)
			| #"D" =>
				update_c(arr,time_array,cats,water,i-2,j,#"U",t,n,m,cats_num,water_num,water_prev)
			| #"U" =>
		...
		and update_cat (arr,time_array,cats,water,t,n,m,cats_num,water_num,water_prev) =
			if Queue.isEmpty(cats) then answer(arr,time_array,cats,water,t,n,m,cats_num,water_num,water_prev)(*update_water(arr,time_array,cats,water,t,n,m,cats_num,water_num,water_prev)*)
			else
			let

...
string Answer (char board[1000][1000], int ti, int tj, int si, int sj){
    string answer ;
    string key;
        while(!(ti==si && tj==sj)){
...
void min_pos(int result [2], char board[1000][1000], int M, int N){
    for (int i=0; i<M; i++){
        for (int j=0; j<N; j++){
...
void Min_pos(int result [2], char board[1000][1000],int timed[1000][1000], int M, int N,int T){
    for (int i=0; i<M; i++){
        for (int j=0; j<N; j++){
...
bool update_cat (char board[1000][1000],int timed[1000][1000],int bs[2] ,int I,int J,int T,int N,int M){
    bool cat=false;
    if(J!=0){
        if(board[I][J-1] == '.' ){
            board[I][J-1] = 'L';
...
void update_water (char board[1000][1000],int timed[1000][1000],int bs[2] , int I,int J,int T,int N,int M){
    if(J!=0){
    ...
    while(play){
        ...
        for(i=0; i<M; i++){
            for(j=0; j<N; j++){
                ...
                if( is_cat(Board[i][j]) ){
                    if (Timed[i][j]==time-1){
                        temp=update_cat(Board,Timed,Bs,i,j,time,N,M);
                        if(temp){
                            Stop = false;
                        }
                    }
        ...
        for(i=0; i<M; i++){
            for(j=0; j<N; j++){
                if(is_water(Board[i][j]) && Timed[i][j]== time-1){
                    update_water(Board,Timed,Bs,i,j,time,N,M);
                }
            }
        }
        time++;
